...
import { getResults, getRoadmapPreview, RoleMatch, RoadmapPreview } from "@/lib/api";
...
export default function ResultsPage() {
  ...
  const [roadmapPreviews, setRoadmapPreviews] = useState<Record<number, RoadmapPreview>>({});
  ...
  const loadResults = async (veteranId: number) => {
    try {
      ...
      // Load roadmap previews for each result
      const previews: Record<number, RoadmapPreview> = {};
      await Promise.all(
        data.map(async (match) => {
          try {
            const preview = await getRoadmapPreview(match.role.id);
            previews[match.role.id] = preview;
          } catch {
            // Silently fail if preview not available
          }
        })
      );
      setRoadmapPreviews(previews);
                    ...
                    <div className="p-5 pt-6">
                      ...
                      {isExpanded && (
                        <div className="mt-4 pt-4 border-t border-avery-bg-hover animate-fade-in">
                          ...
                          {/* Roadmap Preview */}
                          {roadmapPreviews[match.role.id] && (
                            <div className="mt-4 pt-4 border-t border-avery-bg-hover">
                              <p className="text-xs text-avery-text-muted uppercase tracking-wider mb-3">
                                Your Path Forward
                              </p>
                              <div className="bg-avery-bg-subtle rounded-lg p-4 space-y-3">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <p className="text-sm font-medium text-avery-text-primary">
                                      {roadmapPreviews[match.role.id].top_certification}
                                    </p>
                                    <p className="text-xs text-avery-text-muted">
                                      Recommended first certification
                                    </p>
                                  </div>
                                </div>
                                <div className="flex items-center justify-between text-sm">
                                  <span className="text-avery-text-secondary">
                                    {roadmapPreviews[match.role.id].quick_summary}
                                  </span>
                                </div>
                              </div>
                              ...
                              {/* Start This Path CTA */}
                              <Link
                                href={`/roadmap/${match.role.id}?match=${Math.round(match.match_percentage)}`}
                                className="block mt-4"
                                onClick={(e) => e.stopPropagation()}
                              >
                                <div
                                  className={`
                                    w-full py-3 px-4 rounded-xl text-center font-semibold
                                    bg-gradient-to-r from-avery-cyan to-avery-teal
                                    text-avery-bg hover:opacity-90 transition-opacity
                                    shadow-lg shadow-avery-cyan/20
                                  `}
                                >
                                  Start This Path
                                </div>
